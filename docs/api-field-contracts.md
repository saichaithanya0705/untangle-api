...
This document is the human-readable contract for request fields currently accepted by the gateway. When docs and implementation diverge, the implementation is authoritative; this file should be updated to match it.
...
2. `api.compatibility.normalizeLegacyParams=true`
   Applies legacy-field normalization for the supported data-plane endpoints below.
...
2. `input` (string or string array) -> `messages` when `messages` is missing.
...
3. `prompt` -> `input` when `input` is missing.
...
Allowed per-record fields in `records[]`:
...
`toleranceUsd` as a finite number
